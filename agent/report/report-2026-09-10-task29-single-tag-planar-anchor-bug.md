...
> **结论作废（2026-09-10 后续复核）**：本报告把非零 tilt 下
> `planar_xy_yaw(C_full)` 的单点投影差误判成代码 Bug，并据此引入 `t=P-Rz(yaw)Q` 首次重锚。
> 用户真机复测表明误差由约 `(0.20,0.10)m` 恶化至 `(0.25,0.15)m`；Task29 规范实际明确要求
> 首次沿用 Task27 的 `C_full` x/y/yaw。本报告仅保留为错误过程记录，不得作为当前算法或结论
> 依据；后续复原及验证见同日新的 Task27 首次算法复原报告。
