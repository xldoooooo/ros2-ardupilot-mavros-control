...
本文件。当前基线日期为 2026-09-17，仓库线协议为 3.2。
...
- 2026-09-17 启动器已用持续状态订阅 + GNU timeout 相对定时替换受校时影响的 Bash SECONDS，
  ...
  --check 单轮由 6.223 降至 1.727 秒；当前正式启动 READY 对照为 28.867 秒（参数事件版历史
  33.76/32.45 秒，最初 54.14 秒），未解锁；整表仍约 36 秒。启动期抖动见 parameter-event 报告。
  独立按名称只读请求实验，两参数约 0.11 秒返回，READY 为 7.497/7.643 秒；尚未接入生产，
  不能将该时间当作正式启动能力。插件裁剪只形成同日 mavros-plugin-trimming-plan 报告，未实施。
...
- UQ212 同板内参标定入口已部署 refresh：`/home/nvidia/camera_int_calib/uq212_cam_calib.py`，
  ...
  用户要求只做相机适配，算法以原 wainstek_cam_calib.py 为基线；已恢复每2帧检测及
  USE_RATIONAL_MODEL=False 开关（保留原可选行为），不擅自调整质量阈值/采样/求解逻辑。
  移动出画面时，越界/空/非有限角点及无效单应矩阵会跳过本帧而不退出；手动强采也不得纳入
  几何无效帧。该边缘负切片崩溃修复已部署 refresh，开发机/机载11项回归通过。
  ...
  合成标定与离屏GUI退出检查通过；用户已完成上述30张实测并明确授权投入生产。
  refresh 稀疏检出已加入工具与测试路径；new 飞机尚未部署该入口。
