...
- 2026-09-23 new被动录制160帧Odin点云与6228条里程计，状态采样均未解锁；实录回放因近身点距
  ...
  配置模板维护单位/约束说明，内部数值策略集中为具名常量；后续注释及常量命名整理尚未部署new，
  未改变默认参数，new运行验收基线仍为上述Task36初始提交。
- 2026-09-24 new独立库定点新增并构建 `odin.launch.py`（未整体升级旧源码），使用
  `odom`、接收时间新鲜度及 z 下界 -2m，修复现场帧/时间源/起点越界问题。
  实时台架仍返回NO_PATH：单帧原始点距起点约0.102m、目标(4,0,1)约0.096m，
  均小于约0.648m有效安全半径；未放宽碰撞判定，没有验证到成功轨迹。refresh未部署此入口。
- 同日独立库 `e8472ac` 增加启动时配置的三维盲区：默认关闭，按源时间戳匹配里程计、
  支持 child-frame 球心偏移，累计前过滤；全部过滤报告NO_MAP，`/filtered_cloud`可视化当前帧。
  new定点更新wrapper/配置/测试/launch并重建，台架以 `odin.launch.py blind_radius:=0.5` 运行；
  零偏移以IMU里程计原点为球心，未标定实际雷达偏移。5帧每帧过滤2139～2231点，最近保留点
  0.503～0.508m，仍NO_PATH；未改约0.648m碰撞半径。new核心仍为初始版本，refresh未部署。
- 用户现场调整位置后已出现有效路径；2026-09-24后续目标(-1.428704,0.970726,1.54494)
  曾连续10秒100次REACH_END。按用户要求，ROS模板预算由0.08升为0.3秒（b69fe4f），
  new已重建并重启，参数读回0.3、盲区0.5，恢复该目标得到REACH_END；原目标未在新预算下复验。
  定时器仍10Hz，但长搜索不能保证10Hz输出；核心库直接调用默认仍0.08秒，refresh未同步。
...
- 当前飞行线协议 `INTERFACE_VERSION = "3.3"`；`guided_interfaces` 与 `onboard_control` 包版本均为
  `3.3.0`。地面站、共享接口和机载运行产物必须同步部署。
