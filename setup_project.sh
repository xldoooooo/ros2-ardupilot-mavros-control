#!/usr/bin/env bash
# Build and verify this checkout on Ubuntu 22.04/Humble or 24.04/Jazzy.
...
[[ -r "${runtime_helpers}" ]] || {
  echo "[project-setup] runtime discovery helper is missing" >&2
  exit 1
}
...
for command in colcon cmake g++ python3; do
  command -v "${command}" >/dev/null 2>&1 || {
    echo "[project-setup] required command is missing: ${command}" >&2
    exit 1
  }
done
...
if ((${#missing_packages[@]} > 0)); then
  echo "[project-setup] missing ROS packages: ${missing_packages[*]}" >&2
  echo "[project-setup] install their ${ROS_DISTRO} packages after review, then rerun" >&2
  exit 1
fi

if [[ ! -x "${project_root}/.venv/bin/python3" ]]; then
  echo "[project-setup] creating project Python environment: .venv"
...
echo "[project-setup] setup passed; run: bash start_ground_all.sh"
