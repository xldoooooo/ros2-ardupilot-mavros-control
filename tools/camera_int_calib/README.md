...
- 保持原算法每2帧检测、全部质量阈值与求解条件。保留 `USE_RATIONAL_MODEL=False`
  原开关，默认估计 fx、fy、cx、cy、k1、k2、p1、p2、k3，不使用理论89°内参作真值。
  不在去畸变预览中强采旧角点；隔帧手动采样沿用最近检测结果，并保存与之对应的原始帧。
...
- A：切换自动采样。
- 空格：强制采纳当前原始画面的检测，绕过清晰度/去重门限，谨慎使用；越界角点或
  无效几何仍拒绝。板移动出画面时跳过当前帧，恢复有效画面后继续，已有样本保留。
