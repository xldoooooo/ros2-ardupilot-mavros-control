...
UQ212 外参自 2026-09-17 按机械尺寸近似：基座位于模块前表面下边缘宽度中点前方
41 mm、下方 46 mm，镜头中心再向下 12 mm，横向居中。结合 Odin 官方 IMU 深度
5.57 mm、离底面 9.26 mm、正视图距左边缘 21.03 mm 和模块宽度 100 mm，得到
`t_imu_camera=(0.04657,0.02897,-0.06726)m`。光轴沿 IMU -Z、画面上方沿 +X、画面右方沿 -Y；
...
印刷纸张有额外黑色外框时，OpenCV 4.6 默认候选合并距离可能将内部 Tag 丢弃，表现为
静止时仍间歇“未检测到 Tag”。检测器显式设置 `minMarkerDistanceRate=0.02`，保留内部
编码框参与解码，不改变纠错或质量门。`size_m` 必须测量**编码区域最外侧黑边**的边长，
不能测装饰外框或整张纸；配置多个 Tag 不改变图像解码。同帧多个 Tag 仍按现有规则拒绝。
解码后，角点精修半径按 `min(7, floor(最短像素边长 / 16))` 选择（至少1px）：
36h11含黑边共8格，窗口半径不超过半格，避免9.9cm小Tag的固定7px窗口跨入邻格或外框。
检测率、重投影残差与短时稳定性都不能替代独立的公制位置精度验收。
...
仓库中面向 refresh 飞机的活动配置对应 UQ212：1920×1080 MJPEG 仅使用设备原生声明的 120 fps，
镜头控制显式恢复 UQ212 驱动报告的默认值。内参采用用户授权的
`uq212_runs/20260917-011117-295510` 原始30张标定结果，fx/fy=942.1241/942.3490px，
RMS=0.8863px，使用全部五个实测畸变系数；未剔除20/21号样本。独立精度仍待验证，
已知异常角点和边缘覆盖不足的风险保留。外参仍使用上述机械尺寸与垂直光轴近似。旧 Wasintek 的
配置保存在 `config/Wasintek/`，当前 UQ212 配置档案保存在 `config/UQ212/`；两个命名目录都不
参与运行时加载，根 `config/` 下的同名文件才是当前活动配置。

### 采集与发布帧率

`camera.conf` 的 `camera.fps` 只请求相机原生模式；`camera.publish_fps` 独立限制 MJPEG
解码及 ROS 图像发布的平均频率，默认 `30.0`，`0` 表示不限频。只允许有限非负值；旧配置
省略此项时也使用 30。修改后重启独立修正服务生效，无需修改标定检测频率或相机内外参。
例如 UQ212 保留 `fps = 120`，同时设置 `publish_fps = 30.0`。

节点持续读取并归还 V4L2 缓冲区，按真实采集单调时间节拍在解码前丢弃多余帧，不通过休眠限频，
不积压旧图、不补发、不生成重复帧，也不改写采集时间戳。源帧低于上限时按实际可用帧率输出。
限频控制长期平均频率，离散源帧和抖动可使相邻输出间隔不完全均匀。`CAMERA_HEALTH` 中的
`rate_limited` 是主动限频丢弃数，区别于过期/非单调丢帧。USB 原始带宽不会因此降低；标定仍
遵循 `processing_rate_hz`（当前 10 Hz），更低的发布上限可能延长收敛，因此应按场景验证。

### 更换下视相机时修改哪里

设备入口是根活动配置 [camera.conf](config/camera.conf) 中的 `camera.device`。应优先填写
`/dev/v4l/by-id/...-video-index0`；它比 `/dev/videoN` 稳定。地面站标定面板本身不枚举飞机 USB
设备，而是连接 `correction_service`。服务只在收到 `first/next` 时按该路径打开相机，因此设备
路径正确、服务重启并重新发布状态后，面板即可使用新设备。

- 仅同一颗相机换 USB 口且 by-id 不变：无需改配置；
- 同型号相机但 by-id/序列号变化：只改根 `camera.conf` 的 `device`，然后重启独立修正服务；
- 更换相机型号或镜头：在 `config/<新相机名>/` 保存该型号的 `camera.conf`、
  `intrinsics.yaml`、`extrinsics.yaml`、`lens.conf`，再把这四份核验后的内容复制为根活动配置并
  重启独立修正服务。不得只改设备路径后继续沿用未知内外参或不兼容的 V4L2 控制；
- 纯配置变更无需重新编译，但会改变配置指纹；重启后旧窗口/候选不得继续使用。

可在飞机上先执行 `v4l2-ctl --list-devices`、`--list-formats-ext` 和 `--list-ctrls-menus` 核对路径、
模式和镜头控制。面板目前没有远程编辑配置或自动切换命名档案的功能，避免在校准过程中静默换用
另一套几何参数。
...
标定配置中 `source_path` 仅记录外参数据或机械推导报告的来源，不在运行时读取那个路径。
...
- 2026-09-18 refresh 现场已配置 Tag0 `(0,0)`、Tag1 `(0,1.8)`、边长均为0.099m，日志
  记录了用户完成两点采样及应用；这是流程成功记录，尚无独立真值精度验收。现场 CSV
  是用户维护的配置，不应被部署覆盖。间歇识别排查与修复见当日 Tag 外框检测报告。
