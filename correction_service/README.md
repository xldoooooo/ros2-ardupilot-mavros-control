...
0.080/0.150 m；候选最大年龄 300 s；窗口最长 1800 s、相邻观测最长 600 s；实际 FCU
中心位置/yaw 跳变上限 1.0 m/10°。这些是原理验证的保守工程门限，不是当前硬件精度测量值。
...
- 主任务说明下视相机已安装且可见 Tag 0，但本次未连接飞机复核。相机曾拆装，当前外参可能
  偏离旧标定；云台同型号相机绝不能替代下视校准相机。固定碳板并重做外参、布设精测多 Tag、
  引入独立真值并跨 session 重复试验前，真实精度一律记为“未验证”。
...
- 水平 SE(2) 假设不能掩盖高度相关的外参误差或轨迹漂移；tilt、窗口跨度和残差超限时应拒绝，
  不能放宽门限或修正 z 来美化结果。
