...
当前 UQ212 临时沿用的是 2026-08-27 Wasintek 标定矩阵，不代表新相机已完成外参标定；在
UQ212 重新标定前，安装位置、光心变化和小角度误差都属于未验证风险。
...
refresh 飞机当前活动配置对应 UQ212：1920×1080 MJPEG 仅使用设备原生声明的 120 fps，
镜头控制显式恢复 UQ212 驱动报告的默认值。内参暂按 89°对角视场、方形像素、中心主点和零畸变
构造，外参暂时沿用上一颗 Wasintek 相机；两者都不是 UQ212 实测标定结果。旧 Wasintek 的
