...
smoke_test() {
  ...
  grep -Eq "^interface_version: ['\"]?2\\.1['\"]?$" "${status_log}" ||
    die "smoke status did not report interface version 2.1"
  ...
  log "smoke passed: interface=2.1, fcu_connected=false, armed=false, setpoint_messages=0"
  cleanup_smoke
  trap - EXIT INT TERM
}
